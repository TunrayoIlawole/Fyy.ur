...
def venues():
  ...
  all_places = Venue.query.order_by(Venue.city, Venue.State).all()
  data = []

  for place in all_places:
    # Get all the venues for a particular state and city
    filtered_venues = Venue.query.filter_by(city=place.city).filter_by(state=place.state).all()
    venues = []
    ...
    for venue in filtered_venues:
      ...
      num_upcoming_shows = len(db.session.query(Show).filter(Show.venue_id==venue.id).filter_by(Show.start_time > datetime.now()).all())
      venues.append({
        'id': venue.id,
        "name": venue.name,
...
def search_venues():
  ...
  for result in search_results:
    num_upcoming_shows = len(db.session.query(Show).filter(Show.venue_id==result.id).filter_by(Show.start_time > datetime.now()).all())
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": num_upcoming_shows
...
def create_venue_submission():
  ...
  try:
    ...
    venue.seeking_talent = True if 'venue.seeking_talent' in request.form else False
...
def search_artists():
  ...
  data = []
  for result in search_results:
    num_upcoming_shows = len(db.session.query(Show).filter(Show.artist_id==result.id).filter_by(Show.start_time > datetime.now()).all())
    data.append({
      "id": result.id,
      "name": result.name,
      "num_upcoming_shows": num_upcoming_shows
...
def show_artist(artist_id):
  ...
  all_past_shows = db.session.query(Show).join(Venue).filter(Show.artist_id==artist_id).filter(Show.start_time < datetime.now()).all()
